...
{
   public:
      market_history_api_impl( bearshares::app::application& _app )
         :app( _app ) {}
...
market_ticker market_history_api_impl::get_ticker() const
{
   ...
   {
      auto open = ( asset( itr->open_bsd, BSD_SYMBOL ) / asset( itr->open_bears, BEARS_SYMBOL ) ).to_real();
      result.latest = ( asset( itr->close_bsd, BSD_SYMBOL ) / asset( itr->close_bears, BEARS_SYMBOL ) ).to_real();
...
market_volume market_history_api_impl::get_volume() const
{
   ...
   do
   {
      result.bears_volume.amount += itr->bears_volume;
      result.bsd_volume.amount += itr->bsd_volume;
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< bearshares::chain::limit_order_index >().indices().get< bearshares::chain::by_price >();
   auto itr = order_idx.lower_bound( price::max( BSD_SYMBOL, BEARS_SYMBOL ) );

   order_book result;

   while( itr != order_idx.end() && itr->sell_price.base.symbol == BSD_SYMBOL && result.bids.size() < limit )
   {
      order cur;
   ...
   while( itr != order_idx.end() && itr->sell_price.base.symbol == BEARS_SYMBOL && result.asks.size() < limit )
   {
      order cur;
...
market_history_api::market_history_api( const bearshares::app::api_context& ctx )
{
