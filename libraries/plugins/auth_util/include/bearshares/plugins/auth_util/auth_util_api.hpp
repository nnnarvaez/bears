...
{
   public:
      auth_util_api( const bearshares::app::api_context& ctx );
...
FC_REFLECT( bearshares::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( bearshares::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( bearshares::plugin::auth_util::auth_util_api,
