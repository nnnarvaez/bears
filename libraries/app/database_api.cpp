...
#include <bearshares/app/database_api.hpp>
...
{
   public:
      database_api_impl( const bearshares::app::api_context& ctx  );
      ~database_api_impl();
      ...
      bearshares::chain::database&                _db;
      std::shared_ptr< bearshares::follow::follow_api > _follow_api;
...
database_api::database_api( const bearshares::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const bearshares::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< bearshares::follow::follow_api >( ctx );
   }
...
{
   return bearshares::protocol::get_config();
}
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<bearshares::chain::account_member_index>();
   ...
   return result;*/
   FC_ASSERT( false, "database_api::get_account_references --- Needs to be refactored for bears." );
}
...
vector< withdraw_route > database_api::get_withdraw_routes( string account, withdraw_route_type type )const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      {
         const auto& by_route = my->_db.get_index< withdraw_coining_route_index >().indices().get< by_withdraw_route >();
      ...
      {
         const auto& by_dest = my->_db.get_index< withdraw_coining_route_index >().indices().get< by_destination >();
...
//   if( buy_itr != end ) idump((*buy_itr));
   ...
   while(  sell_itr != end && sell_itr->sell_price.base.symbol == BSD_SYMBOL && result.bids.size() < limit )
   {
      auto itr = sell_itr;
      order cur;
   ...
   while(  buy_itr != end && buy_itr->sell_price.base.symbol == BEARS_SYMBOL && result.asks.size() < limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
...
//   wdump((trx)(available_keys));
   auto result = trx.get_required_signatures( BEARSHARES_CHAIN_ID,
                                              available_keys,
...
{
   trx.verify_authority( BEARSHARES_CHAIN_ID,
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).active  ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).owner   ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).posting ); },
                         BEARSHARES_MAX_SIG_CHECK_DEPTH );
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   asset pot;
   if( my->_db.has_hardfork( BEARSHARES_HARDFORK_0_17__774 ) )
   ...
   if( total_r2 > 0 )
   {
      ...
      if( my->_db.has_hardfork( BEARSHARES_HARDFORK_0_17__774 ) )
      {
         ...
         vshares = d.net_rshares.value > 0 ? bearshares::chain::util::evaluate_reward_curve( d.net_rshares.value, rf.author_reward_curve, rf.content_constant ) : 0;
      }
      else
         vshares = d.net_rshares.value > 0 ? bearshares::chain::util::evaluate_reward_curve( d.net_rshares.value ) : 0;
   ...
   if( d.parent_author != BEARSHARES_ROOT_POST_PARENT )
                                                  ...
                                                  )const
{
   ...
   while( count > 0 && tidx_itr != tidx.end() )
   {
      ...
      try
      {
         ...
         result.back().promoted = asset(tidx_itr->promoted_balance, BSD_SYMBOL );
...
vector<discussion> database_api::get_discussions_by_promoted( const discussion_query& query )const
{
   ...
   return my->_db.with_read_lock( [&]()
   {
      ...
      auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type(BEARSHARES_MAX_SHARE_SUPPLY) )  );
...
vector< coining_delegation_api_obj > database_api::get_coining_delegations( string account, string from, uint32_t limit )const
{
   FC_ASSERT( limit <= 1000 );

   return my->_db.with_read_lock( [&]()
   {
      vector< coining_delegation_api_obj > result;
      result.reserve( limit );

      const auto& delegation_idx = my->_db.get_index< coining_delegation_index, by_delegation >();
...
vector< coining_delegation_expiration_api_obj > database_api::get_expiring_coining_delegations( string account, time_point_sec from, uint32_t limit )const
{
   FC_ASSERT( limit <= 1000 );

   return my->_db.with_read_lock( [&]()
   {
      vector< coining_delegation_expiration_api_obj > result;
      result.reserve( limit );

      const auto& exp_idx = my->_db.get_index< coining_delegation_expiration_index, by_account_expiration >();
