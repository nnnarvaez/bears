...
{
   FC_ASSERT( permlink.size() > BEARSHARES_MIN_PERMLINK_LENGTH && permlink.size() < BEARSHARES_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void validate_auth_size( const authority& a )
{
   ...
   FC_ASSERT( size <= BEARSHARES_MAX_AUTHORITY_MEMBERSHIP, "Authority membership exceeded. Max: 10 Current: ${n}", ("n", size) );
}
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( BEARSHARES_HARDFORK_0_1 ) )
   {
      FC_ASSERT( o.url.size() <= BEARSHARES_MAX_WITNESS_URL_LENGTH, "URL is too long" );
   }
   else if( o.url.size() > BEARSHARES_MAX_WITNESS_URL_LENGTH )
   {
   ...
   if ( _db.has_hardfork( BEARSHARES_HARDFORK_0_14__410 ) )
   {
      FC_ASSERT( o.props.account_creation_fee.symbol == BEARS_SYMBOL );
   }
   else if( o.props.account_creation_fee.symbol != BEARS_SYMBOL )
   {
      // after HF, above check can be moved to validate() if reindex doesn't show this warning
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_19__987) )
   {
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
      FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * BEARSHARES_CREATE_ACCOUNT_WITH_BEARS_MODIFIER, BEARS_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
                 ("f", wso.median_props.account_creation_fee * asset( BEARSHARES_CREATE_ACCOUNT_WITH_BEARS_MODIFIER, BEARS_SYMBOL ) )
                 ("p", o.fee) );
   }
   else if( _db.has_hardfork( BEARSHARES_HARDFORK_0_1 ) )
   {
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
   ...
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_15__465 ) )
   {
   ...
   const auto& new_account = _db.create< account_object >( [&]( account_object& acc )
   {
      ...
      if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_11__169 ) )
...
{
   FC_ASSERT( _db.has_hardfork( BEARSHARES_HARDFORK_0_17__818 ), "Account creation with delegation is not enabled until hardfork 17" );
   ...
   auto target_delegation = asset( wso.median_props.account_creation_fee.amount * BEARSHARES_CREATE_ACCOUNT_WITH_BEARS_MODIFIER * BEARSHARES_CREATE_ACCOUNT_DELEGATION_RATIO, BEARS_SYMBOL ) * props.get_coining_share_price();

   auto current_delegation = asset( o.fee.amount * BEARSHARES_CREATE_ACCOUNT_DELEGATION_RATIO, BEARS_SYMBOL ) * props.get_coining_share_price() + o.delegation;
   ...
   if( o.delegation.amount > 0 || !_db.has_hardfork( BEARSHARES_HARDFORK_0_19__997 ) )
   {
      _db.create< coining_delegation_object >( [&]( coining_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + BEARSHARES_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
{
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_1 ) ) FC_ASSERT( o.account != BEARSHARES_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( BEARSHARES_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();
   ...
   {
#ifndef IS_TEST_NET
      if( _db.has_hardfork( BEARSHARES_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > BEARSHARES_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( BEARSHARES_HARDFORK_0_15__465 ) ) )
   {
      for( auto a: o.active->account_auths )
      {
         _db.get_account( a.first );
   ...
   if( o.posting && ( _db.has_hardfork( BEARSHARES_HARDFORK_0_15__465 ) ) )
   {
      for( auto a: o.posting->account_auths )
      {
         _db.get_account( a.first );
...
{
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_10 ) )
   {
   ...
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_6__80 ) && comment.parent_author != BEARSHARES_ROOT_POST_PARENT )
   {
...
{ try {
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_5__55 ) )
   ...
   if( o.parent_author != BEARSHARES_ROOT_POST_PARENT )
   {
      parent = &_db.get_comment( o.parent_author, o.parent_permlink );
      if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < BEARSHARES_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",BEARSHARES_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < BEARSHARES_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",BEARSHARES_MAX_COMMENT_DEPTH) );
   }

   if( ( _db.has_hardfork( BEARSHARES_HARDFORK_0_17__926 ) ) && o.json_metadata.size() )
   ...
   if ( itr == by_permlink_idx.end() )
   {
      if( o.parent_author != BEARSHARES_ROOT_POST_PARENT )
      {
         ...
         if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) && !_db.has_hardfork( BEARSHARES_HARDFORK_0_17__869 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( *parent ) != fc::time_point_sec::maximum(), "Discussion is frozen." );
      }

      if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == BEARSHARES_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > BEARSHARES_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) > BEARSHARES_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( BEARSHARES_HARDFORK_0_6__113 ) )
      {
         if( o.parent_author == BEARSHARES_ROOT_POST_PARENT )
             FC_ASSERT( (now - auth.last_post) > BEARSHARES_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
         else
             FC_ASSERT( (now - auth.last_post) > BEARSHARES_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else
      ...
      uint16_t reward_weight = BEARSHARES_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12__176 ) && !_db.has_hardfork( BEARSHARES_HARDFORK_0_17__733 ) && o.parent_author == BEARSHARES_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), BEARSHARES_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( BEARSHARES_POST_AVERAGE_WINDOW - post_delta_time ) ) / BEARSHARES_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + BEARSHARES_100_PERCENT );
         reward_weight = uint16_t( std::min( ( BEARSHARES_POST_WEIGHT_CONSTANT * BEARSHARES_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( BEARSHARES_100_PERCENT ) ) );
      }
      ...
      const auto& new_comment = _db.create< comment_object >( [&]( comment_object& com )
      {
         ...
         if ( o.parent_author == BEARSHARES_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) ?
               _db.head_block_time() + BEARSHARES_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
         ...
         if( _db.has_hardfork( BEARSHARES_HARDFORK_0_17__769 ) )
         {
            com.cashout_time = com.created + BEARSHARES_CASHOUT_WINDOW_SECONDS;
         }
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
   ...
   else // start edit case
   {
      ...
      if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_17__772 ) )
      {
         if( _db.has_hardfork( BEARSHARES_HARDFORK_0_14__306 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
         else if( _db.has_hardfork( BEARSHARES_HARDFORK_0_10 ) )
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
   try
   {
      ...
      {
         const auto& from_account = _db.get_account( o.from );
         _db.adjust_balance( from_account, escrow.bears_balance );
         _db.adjust_balance( from_account, escrow.bsd_balance );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
   try
   {
      ...
      _db.adjust_balance( receiver_account, o.bears_amount );
      _db.adjust_balance( receiver_account, o.bsd_amount );
...
void transfer_to_coining_evaluator::do_apply( const transfer_to_coining_operation& o )
{
   ...
   FC_ASSERT( _db.get_balance( from_account, BEARS_SYMBOL) >= o.amount, "Account does not have sufficient BEARS for transfer." );
   _db.adjust_balance( from_account, -o.amount );
...
void withdraw_coining_evaluator::do_apply( const withdraw_coining_operation& o )
{
   ...
   if( !account.mined && _db.has_hardfork( BEARSHARES_HARDFORK_0_1 ) )
   {
      const auto& props = _db.get_dynamic_global_properties();
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
      ...
      FC_ASSERT( account.coining_shares > min_coins || ( _db.has_hardfork( BEARSHARES_HARDFORK_0_16__562 ) && o.coining_shares.amount == 0 ),
   ...
   if( o.coining_shares.amount == 0 )
   {
      if( _db.has_hardfork( BEARSHARES_HARDFORK_0_5__57 ) )
   ...
   {
      int coining_withdraw_intervals = BEARSHARES_COINING_WITHDRAW_INTERVALS_PRE_HF_16;
      if( _db.has_hardfork( BEARSHARES_HARDFORK_0_16__551 ) )
         coining_withdraw_intervals = BEARSHARES_COINING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
      ...
      _db.modify( account, [&]( account_object& a )
      {
         ...
         a.next_coining_withdrawal = _db.head_block_time() + fc::seconds(BEARSHARES_COINING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_coining_route_evaluator::do_apply( const set_withdraw_coining_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < BEARSHARES_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= BEARSHARES_100_PERCENT, "More than 100% of coining withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( BEARSHARES_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= BEARSHARES_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( BEARSHARES_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < BEARSHARES_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( BEARSHARES_HARDFORK_0_3 ) ) {
...
{ try {
   ...
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
   {
#ifndef CLEAR_VOTES
   ...
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_11 ) )
      FC_ASSERT( elapsed_seconds >= BEARSHARES_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (BEARSHARES_100_PERCENT * elapsed_seconds) / BEARSHARES_VOTE_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_power + regenerated_power), int64_t(BEARSHARES_100_PERCENT) );
   FC_ASSERT( current_power > 0, "Account currently does not have voting power." );

   int64_t  abs_weight    = abs(o.weight);
   int64_t  used_power    = (current_power * abs_weight) / BEARSHARES_100_PERCENT;

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // used_power = (current_power * abs_weight / BEARSHARES_100_PERCENT) * (reserve / max_vote_denom)
   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * BEARSHARES_VOTE_REGENERATION_SECONDS / (60*60*24);
   FC_ASSERT( max_vote_denom > 0 );

   if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_14__259 ) )
   {
   ...
   int64_t abs_rshares    = ((uint128_t(voter.effective_coining_shares().amount.value) * used_power) / (BEARSHARES_100_PERCENT)).to_uint64();
   if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;

   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_14__259 ) )
   {
      FC_ASSERT( abs_rshares > BEARSHARES_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or bears power." );
   }
   else if( _db.has_hardfork( BEARSHARES_HARDFORK_0_13__248 ) )
   {
      FC_ASSERT( abs_rshares > BEARSHARES_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or bears power." );
   }
   ...
   {
      if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) )
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         if( _db.has_hardfork( BEARSHARES_HARDFORK_0_17__900 ) )
            FC_ASSERT( _db.head_block_time() < comment.cashout_time - BEARSHARES_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
         else if( _db.has_hardfork( BEARSHARES_HARDFORK_0_7 ) )
            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - BEARSHARES_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
      }
      ...
      if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) && !_db.has_hardfork( BEARSHARES_HARDFORK_0_13__257)  )
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + BEARSHARES_CASHOUT_WINDOW_SECONDS_PRE_HF17;
         else
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + BEARSHARES_CASHOUT_WINDOW_SECONDS_PRE_HF12;
      ...
      _db.modify( comment, [&]( comment_object& c ){
         ...
         if( rshares > 0 )
            ...
            c.net_votes--;
         if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
      });
      ...
      _db.modify( root, [&]( comment_object& c )
      {
         ...
         if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_17__769 ) )
         {
            if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
            ...
            if( c.max_cashout_time == fc::time_point_sec::maximum() )
               c.max_cashout_time = _db.head_block_time() + fc::seconds( BEARSHARES_MAX_CASHOUT_WINDOW_SECONDS );
         }
      });
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible && _db.has_hardfork( BEARSHARES_HARDFORK_0_17__774 ) )
            curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;

         if( curation_reward_eligible )
         {
            if( comment.created < fc::time_point_sec(BEARSHARES_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
               ...
               if( _db.has_hardfork( BEARSHARES_HARDFORK_0_17__774 ) )
               {
                  const auto& reward_fund = _db.get_reward_fund( comment );
                  auto curve = !_db.has_hardfork( BEARSHARES_HARDFORK_0_19__1052 ) && comment.created > BEARSHARES_HF_19_SQRT_PRE_CALC
            ...
            if( _db.head_block_time() > fc::time_point_sec(BEARSHARES_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
               /// discount weight by time
               uint128_t w(max_vote_weight);
               uint64_t delta_t = std::min( uint64_t((cv.last_update - comment.created).to_seconds()), uint64_t(BEARSHARES_REVERSE_AUCTION_WINDOW_SECONDS) );

               w *= delta_t;
               w /= BEARSHARES_REVERSE_AUCTION_WINDOW_SECONDS;
   ...
   {
      FC_ASSERT( itr->num_changes < BEARSHARES_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );

      if( _db.has_hardfork( BEARSHARES_HARDFORK_0_6__112 ) )
      ...
      if( !_db.has_hardfork( BEARSHARES_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( BEARSHARES_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
void custom_evaluator::do_apply( const custom_operation& o )
{
   ...
   if( d.is_producing() )
   {
      ...
      FC_ASSERT( o.required_auths.size() <= BEARSHARES_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", o.required_auths.size()) );
   }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
   ...
   if( d.is_producing() )
   {
      ...
      FC_ASSERT( num_auths <= BEARSHARES_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", num_auths) );
   }
...
void custom_binary_evaluator::do_apply( const custom_binary_operation& o )
{
   ...
   FC_ASSERT( d.has_hardfork( BEARSHARES_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( BEARSHARES_HARDFORK_0_5__59 ) )
   {
   ...
   if(itr == accounts_by_name.end())
   {
      db.create< account_object >( [&]( account_object& acc )
      {
         ...
         if( !db.has_hardfork( BEARSHARES_HARDFORK_0_11__169 ) )
            acc.recovery_account = "bears";
         else
   ...
   if( db.has_hardfork( BEARSHARES_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < BEARSHARES_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= BEARSHARES_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( BEARSHARES_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( BEARSHARES_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( BEARSHARES_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= BEARSHARES_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  auto bearshares_conversion_delay = BEARSHARES_CONVERSION_DELAY_PRE_HF_16;
  if( _db.has_hardfork( BEARSHARES_HARDFORK_0_16__551) )
     bearshares_conversion_delay = BEARSHARES_CONVERSION_DELAY;
  ...
  _db.create<convert_request_object>( [&]( convert_request_object& obj )
  {
      ...
      obj.amount          = o.amount;
      obj.conversion_date = _db.head_block_time() + bearshares_conversion_delay;
  });
...
{
   FC_ASSERT( !_db.has_hardfork( BEARSHARES_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void challenge_authority_evaluator::do_apply( const challenge_authority_operation& o )
{
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_14__307 ) ) FC_ASSERT( false, "Challenge authority operation is currently disabled." );
   ...
   if( o.require_owner )
   {
      ...
      FC_ASSERT( challenger.balance >= BEARSHARES_OWNER_CHALLENGE_FEE );
      FC_ASSERT( !challenged.owner_challenged );
      FC_ASSERT( _db.head_block_time() - challenged.last_owner_proved > BEARSHARES_OWNER_CHALLENGE_COOLDOWN );

      _db.adjust_balance( challenger, - BEARSHARES_OWNER_CHALLENGE_FEE );
      _db.create_coining( _db.get_account( o.challenged ), BEARSHARES_OWNER_CHALLENGE_FEE );
  ...
  {
      FC_ASSERT( challenger.balance >= BEARSHARES_ACTIVE_CHALLENGE_FEE, "Account does not have sufficient funds to pay challenge fee." );
      FC_ASSERT( !( challenged.owner_challenged || challenged.active_challenged ), "Account is already challenged." );
      FC_ASSERT( _db.head_block_time() - challenged.last_active_proved > BEARSHARES_ACTIVE_CHALLENGE_COOLDOWN, "Account cannot be challenged because it was recently challenged." );

      _db.adjust_balance( challenger, - BEARSHARES_ACTIVE_CHALLENGE_FEE );
      _db.create_coining( _db.get_account( o.challenged ), BEARSHARES_ACTIVE_CHALLENGE_FEE );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + BEARSHARES_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( BEARSHARES_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > BEARSHARES_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + BEARSHARES_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < BEARSHARES_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + BEARSHARES_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( BEARSHARES_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + BEARSHARES_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void delegate_coining_shares_evaluator::do_apply( const delegate_coining_shares_operation& op )
{
   ...
   auto available_shares = delegator.coining_shares - delegator.delegated_coining_shares - asset( delegator.to_withdraw - delegator.withdrawn, COINS_SYMBOL );

   const auto& wso = _db.get_witness_schedule_object();
   const auto& gpo = _db.get_dynamic_global_properties();
   auto min_delegation = asset( wso.median_props.account_creation_fee.amount * 10, BEARS_SYMBOL ) * gpo.get_coining_share_price();
   auto min_update = wso.median_props.account_creation_fee * gpo.get_coining_share_price();

   // If delegation doesn't exist, create it
   if( delegation == nullptr )
   {
      FC_ASSERT( available_shares >= op.coining_shares, "Account does not have enough coining shares to delegate." );
   ...
   else if( op.coining_shares >= delegation->coining_shares )
   {
      ...
      FC_ASSERT( available_shares >= op.coining_shares - delegation->coining_shares, "Account does not have enough coining shares to delegate." );
   ...
   else /* delegation->coining_shares > op.coining_shares */
   {
      ...
      _db.create< coining_delegation_expiration_object >( [&]( coining_delegation_expiration_object& obj )
      {
         ...
         obj.expiration = std::max( _db.head_block_time() + BEARSHARES_CASHOUT_WINDOW_SECONDS, delegation->min_delegation_time );
      });
