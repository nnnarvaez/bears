...
using bearshares::protocol::block_id_type;
using bearshares::protocol::transaction_id_type;
using bearshares::protocol::chain_id_type;
using bearshares::protocol::account_name_type;
...
{
   class variant;
   inline void to_variant( const bearshares::chain::shared_string& s, variant& var )
   {
      var = fc::string( bearshares::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, bearshares::chain::shared_string& s )
   {
   ...
   namespace raw
   {
      ...
      template< typename T > inline void pack( bearshares::chain::buffer_type& raw, const T& v )
      {
      ...
      template< typename T > inline void unpack( const bearshares::chain::buffer_type& raw, T& v )
      {
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
      }

      template< typename T > inline T unpack( const bearshares::chain::buffer_type& raw )
      {
         T v;
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
...
FC_REFLECT_ENUM( bearshares::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
FC_REFLECT_TYPENAME( bearshares::chain::shared_string )
FC_REFLECT_TYPENAME( bearshares::chain::buffer_type )

FC_REFLECT_ENUM( bearshares::chain::bandwidth_type, (post)(forum)(market) )
