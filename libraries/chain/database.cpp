...
#include <bearshares/chain/block_summary_object.hpp>
#include <bearshares/chain/compound.hpp>
#include <bearshares/chain/custom_operation_interpreter.hpp>
#include <bearshares/chain/database.hpp>
#include <bearshares/chain/database_exceptions.hpp>
#include <bearshares/chain/db_with.hpp>
#include <bearshares/chain/evaluator_registry.hpp>
#include <bearshares/chain/global_property_object.hpp>
#include <bearshares/chain/history_object.hpp>
#include <bearshares/chain/index.hpp>
#include <bearshares/chain/bears_evaluator.hpp>
#include <bearshares/chain/bears_objects.hpp>
#include <bearshares/chain/transaction_object.hpp>
#include <bearshares/chain/shared_db_merkle.hpp>
#include <bearshares/chain/operation_notification.hpp>
#include <bearshares/chain/witness_schedule.hpp>

#include <bearshares/chain/util/asset.hpp>
#include <bearshares/chain/util/reward.hpp>
#include <bearshares/chain/util/uint256.hpp>
#include <bearshares/chain/util/reward.hpp>
...
FC_REFLECT( bearshares::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( bearshares::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( bearshares::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
void database::reindex( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t shared_file_size )
{
   try
   {
      ...
      BEARSHARES_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{ try {
   if( !has_hardfork( BEARSHARES_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( BEARSHARES_HARDFORK_0_17__769 ) || comment.parent_author == BEARSHARES_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( BEARSHARES_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(BEARSHARES_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   if( has_hardfork( BEARSHARES_HARDFORK_0_5__54 ) )
   {
      const auto& witness = get_witness( witness_owner );

      if( witness.running_version != BEARSHARES_BLOCKCHAIN_VERSION )
         pending_block.extensions.insert( block_header_extensions( BEARSHARES_BLOCKCHAIN_VERSION ) );

      const auto& hfp = get_hardfork_property_object();

      if( hfp.current_hardfork_version < BEARSHARES_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == BEARSHARES_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
         && witness.hardfork_version_vote > BEARSHARES_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= BEARSHARES_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      BEARSHARES_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   BEARSHARES_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   BEARSHARES_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   BEARSHARES_TRY_NOTIFY( applied_block, block )
}

void database::notify_pre_apply_block( const signed_block& block )
{
   BEARSHARES_TRY_NOTIFY( pre_apply_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   BEARSHARES_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   BEARSHARES_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   BEARSHARES_TRY_NOTIFY( on_applied_transaction, tx )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = BEARSHARES_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / BEARSHARES_BLOCK_INTERVAL + 1;
}
...
std::pair< asset, asset > database::create_bsd( const account_object& to_account, asset bears, bool to_reward_balance )
{
   ...
   try
   {
      ...
      {
         auto to_bsd = ( gpo.bsd_print_rate * bears.amount ) / BEARSHARES_100_PERCENT;
 ...
 * @param BEARS - BEARS to be converted to coining shares
 */
asset database::create_coining( const account_object& to_account, asset bears, bool to_reward_balance )
{
   try
   {
       ...
       */
      asset new_coining = bears * ( to_reward_balance ? cprops.get_reward_coining_share_price() : cprops.get_coining_share_price() );

      modify( to_account, [&]( account_object& to )
      {
         if( to_reward_balance )
         {
            to.reward_coining_balance += new_coining;
            to.reward_coining_bears += bears;
         }
         else
            to.coining_shares += new_coining;
      } );
      ...
      modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_coining_bears += bears;
         }
         else
         {
            props.total_coining_fund_bears += bears;
            props.total_coining_shares += new_coining;
         }
      } );

      if( !to_reward_balance )
         adjust_proxied_witness_votes( to_account, new_coining.amount );

      return new_coining;
   }
   FC_CAPTURE_AND_RETHROW( (to_account.name)(bears) )
}
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( BEARSHARES_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, BEARSHARES_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != BEARSHARES_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= BEARSHARES_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = BEARSHARES_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = BEARSHARES_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( BEARSHARES_HARDFORK_0_2 ) )
      ...
      if( has_hardfork( BEARSHARES_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( BEARSHARES_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( BEARSHARES_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( BEARSHARES_NULL_ACCOUNT );
   ...
   {
      const auto& gpo = get_dynamic_global_properties();
      auto converted_bears = null_account.coining_shares * gpo.get_coining_share_price();

      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         g.total_coining_shares -= null_account.coining_shares;
         g.total_coining_fund_bears -= converted_bears;
      });
...
{
   if( head_block_num() >= BEARSHARES_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
{
   const auto& widx = get_index< account_index >().indices().get< by_next_coining_withdrawal >();
   const auto& didx = get_index< withdraw_coining_route_index >().indices().get< by_withdraw_route >();
   ...
   while( current != widx.end() && current->next_coining_withdrawal <= head_block_time() )
   {
      ...
      */
           ...
           ++itr )
      {
         if( itr->auto_coin )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / BEARSHARES_100_PERCENT ).to_uint64();
            ...
            if( to_deposit > 0 )
            {
               ...
               push_virtual_operation( fill_coining_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, COINS_SYMBOL ), asset( to_deposit, COINS_SYMBOL ) ) );
            }
         }
      }
           ...
           ++itr )
      {
         if( !itr->auto_coin )
         {
            ...
            if( to_deposit > 0 )
            {
               ...
               push_virtual_operation( fill_coining_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, COINS_SYMBOL), converted_bears ) );
            }
         }
      }
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_coining_withdrawal += fc::seconds( BEARSHARES_COINING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      push_virtual_operation( fill_coining_withdraw_operation( from_account.name, from_account.name, asset( to_withdraw, COINS_SYMBOL ), converted_bears ) );
   }
}
...
void database::adjust_total_payout( const comment_object& cur, const asset& bsd_created, const asset& curator_bsd_value, const asset& beneficiary_value )
{
   ...
   {
      if( c.total_payout_value.symbol == bsd_created.symbol )
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         while( itr != cvidx.end() && itr->comment == c.id )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               auto reward = create_coining( voter, asset( claim, BEARS_SYMBOL ), has_hardfork( BEARSHARES_HARDFORK_0_17__659 ) );
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( BEARSHARES_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / BEARSHARES_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / BEARSHARES_100_PERCENT;
               auto coin_created = create_coining( get_account( b.account ), benefactor_tokens, has_hardfork( BEARSHARES_HARDFORK_0_17__659 ) );
               push_virtual_operation( comment_benefactor_reward_operation( b.account, comment.author, to_string( comment.permlink ), coin_created ) );
               total_beneficiary += benefactor_tokens;
            }

            author_tokens -= total_beneficiary;

            auto bsd_bears     = ( author_tokens * comment.percent_bears_dollars ) / ( 2 * BEARSHARES_100_PERCENT ) ;
            auto coining_bears = author_tokens - bsd_bears;

            const auto& author = get_account( comment.author );
            auto coin_created = create_coining( author, coining_bears, has_hardfork( BEARSHARES_HARDFORK_0_17__659 ) );
            auto bsd_payout = create_bsd( author, bsd_bears, has_hardfork( BEARSHARES_HARDFORK_0_17__659 ) );
            ...
            push_virtual_operation( author_reward_operation( comment.author, to_string( comment.permlink ), bsd_payout.first, bsd_payout.second, coin_created ) );
            push_virtual_operation( comment_reward_operation( comment.author, to_string( comment.permlink ), to_bsd( asset( claimed_reward, BEARS_SYMBOL ) ) ) );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( BEARSHARES_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == BEARSHARES_ROOT_POST_PARENT )
         {
            if( has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + BEARSHARES_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( BEARSHARES_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( BEARSHARES_FIRST_CASHOUT_TIME ) )
      return;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( BEARSHARES_HARDFORK_0_19__1051 ) )
            decay_rate = BEARSHARES_RECENT_RSHARES_DECAY_RATE_HF19;
         else
            decay_rate = BEARSHARES_RECENT_RSHARES_DECAY_RATE_HF17;
   ...
   if( has_hardfork( BEARSHARES_HARDFORK_0_17__771 ) )
   {
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( BEARSHARES_HARDFORK_0_17__771 ) )
      {
         ...
         funds[ fund_id ].bears_awarded += cashout_comment_helper( ctx, *current );
      }
      else
      {
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= funds[ i ].bears_awarded;
         });
      }
   }
}
...
void database::process_funds()
{
   ...
   if( has_hardfork( BEARSHARES_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( BEARSHARES_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / BEARSHARES_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( BEARSHARES_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_bears = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( BEARSHARES_100_PERCENT ) * int64_t( BEARSHARES_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_bears * BEARSHARES_CONTENT_REWARD_PERCENT ) / BEARSHARES_100_PERCENT;
      if( has_hardfork( BEARSHARES_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward ); /// 75% to content creator
      auto coining_reward = ( new_bears * BEARSHARES_COINING_FUND_PERCENT ) / BEARSHARES_100_PERCENT; /// 15% to coining fund
      auto witness_reward = new_bears - content_reward - coining_reward; /// Remaining 10% to witness pay

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= BEARSHARES_MAX_WITNESSES;
      ...
      const auto& producer_reward = create_coining( get_account( cwit.owner ), asset( witness_reward, BEARS_SYMBOL ) );
   ...
   else
   {
      ...
      if( props.head_block_number < BEARSHARES_START_COINING_BLOCK )
...
{
   if( has_hardfork( BEARSHARES_HARDFORK_0_12__178 ) )
      return asset( 0, BEARS_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( BEARSHARES_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< BEARSHARES_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), BEARS_SYMBOL );
   return std::max( percent, BEARSHARES_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( BEARSHARES_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< BEARSHARES_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), BEARS_SYMBOL );
   return std::max( percent, BEARSHARES_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( BEARSHARES_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< BEARSHARES_CURATE_APR_PERCENT >( props.virtual_supply.amount ), BEARS_SYMBOL);
   return std::max( percent, BEARSHARES_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( BEARSHARES_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< BEARSHARES_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), BEARS_SYMBOL);
   auto pay = std::max( percent, BEARSHARES_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in coining shares
   if( props.head_block_number >= BEARSHARES_START_MINER_VOTING_BLOCK || (witness_account.coining_shares.amount.value == 0) ) {
      // const auto& witness_obj = get_witness( props.current_witness );
      const auto& producer_reward = create_coining( witness_account, pay );
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least BEARSHARES_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < BEARSHARES_MAX_WITNESSES && props.head_block_number < BEARSHARES_START_COINING_BLOCK )
      return asset( 0, BEARS_SYMBOL );
#endif

   static_assert( BEARSHARES_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( BEARSHARES_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< BEARSHARES_POW_APR_PERCENT >( props.virtual_supply.amount ), BEARS_SYMBOL);
   return std::max( percent, BEARSHARES_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % BEARSHARES_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( BEARSHARES_HARDFORK_0_8__116 ) )
      return BEARSHARES_1_PERCENT * 25;
   else
      return BEARSHARES_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / BEARSHARES_100_PERCENT;
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + BEARSHARES_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( from_account, old_escrow.bears_balance );
      adjust_balance( from_account, old_escrow.bsd_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, BEARSHARES_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.coining_shares.amount;
      for( int i = 0; i < BEARSHARES_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = BEARSHARES_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(BEARSHARES_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = BEARSHARES_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = BEARSHARES_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = BEARSHARES_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = BEARSHARES_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = BEARSHARES_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = BEARSHARES_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < BEARSHARES_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = BEARSHARES_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, BEARS_SYMBOL );
	    a.bsd_balance  = asset( 10000, BSD_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = BEARSHARES_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = BEARSHARES_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = BEARSHARES_INIT_MINER_NAME;
         p.time = BEARSHARES_GENESIS_TIME;
	 ...
	 p.current_bsd_supply = asset( 10000, BSD_SYMBOL );
      ...
      {
         hpo.processed_hardforks.push_back( BEARSHARES_GENESIS_TIME );
      } );

      // Create witness scheduler
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         wso.current_shuffled_witnesses[0] = BEARSHARES_INIT_MINER_NAME;
      } );
   }
   FC_CAPTURE_AND_RETHROW()
}
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      BEARSHARES_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         BEARSHARES_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
{ try {
   ...
   if( has_hardfork( BEARSHARES_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < BEARSHARES_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",BEARSHARES_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( BEARSHARES_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
try {
   if( (head_block_num() % BEARSHARES_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( BEARSHARES_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_bsd_exchange_update + BEARSHARES_MAX_FEED_AGE_SECONDS
            && !wit.bsd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.bsd_exchange_rate );
         }
      }
      else if( wit.last_bsd_exchange_update < now + BEARSHARES_MAX_FEED_AGE_SECONDS &&
          !wit.bsd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.bsd_exchange_rate );
      }
   ...
   if( feeds.size() >= BEARSHARES_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t bearshares_feed_history_window = BEARSHARES_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( BEARSHARES_HARDFORK_0_16__551) )
            bearshares_feed_history_window = BEARSHARES_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > bearshares_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( BEARSHARES_HARDFORK_0_14__230 ) )
            {
               const auto& gpo = get_dynamic_global_properties();
               price min_price( asset( 9 * gpo.current_bsd_supply.amount, BSD_SYMBOL ), gpo.current_supply ); // This price limits BSD to 10% market cap
...
{ try {
   ...
   const chain_id_type& chain_id = BEARSHARES_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, BEARSHARES_MAX_SIG_CHECK_DEPTH, is_producing() );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         BEARSHARES_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      BEARSHARES_ASSERT( trx.expiration <= now + fc::seconds(BEARSHARES_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",BEARSHARES_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( BEARSHARES_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         BEARSHARES_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      BEARSHARES_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
{ try {
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( has_hardfork( BEARSHARES_HARDFORK_0_14__278 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > BEARSHARES_BLOCKS_PER_DAY )
                  {
   ...
   {
      BEARSHARES_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < BEARSHARES_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",BEARSHARES_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      ...
      if( !median_price.is_null() && has_hardfork( BEARSHARES_HARDFORK_0_14__230 ) )
      {
         auto percent_bsd = uint16_t( ( ( fc::uint128_t( ( dgp.current_bsd_supply * get_feed_history().current_median_history ).amount.value ) * BEARSHARES_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );

         if( percent_bsd <= BEARSHARES_BSD_START_PERCENT )
            dgp.bsd_print_rate = BEARSHARES_100_PERCENT;
         else if( percent_bsd >= BEARSHARES_BSD_STOP_PERCENT )
            dgp.bsd_print_rate = 0;
         else
            dgp.bsd_print_rate = ( ( BEARSHARES_BSD_STOP_PERCENT - percent_bsd ) * BEARSHARES_100_PERCENT ) / ( BEARSHARES_BSD_STOP_PERCENT - BEARSHARES_BSD_START_PERCENT );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < BEARSHARES_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > BEARSHARES_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - BEARSHARES_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( BEARSHARES_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((BEARSHARES_100_PERCENT - BEARSHARES_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / BEARSHARES_100_PERCENT);
...
int database::match( const limit_order_object& new_order, const limit_order_object& old_order, const price& match_price )
{
   ...
   if( !has_hardfork( BEARSHARES_HARDFORK_0_12__178 ) &&
       ( (age >= BEARSHARES_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( BEARSHARES_HARDFORK_0_10__149)) ||
       (age >= BEARSHARES_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( BEARSHARES_HARDFORK_0_10__149) ) ) )
   {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= BEARSHARES_LIQUIDITY_TIMEOUT_SEC )
         {
            r.bsd_volume = 0;
            r.bears_volume = 0;
            r.weight = 0;
         ...
         r.update_weight( has_hardfork( BEARSHARES_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( BEARSHARES_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
...
void database::clear_expired_delegations()
{
   ...
   while( itr != delegations_by_exp.end() && itr->expiration < now )
   {
      ...
      push_virtual_operation( return_coining_delegation_operation( itr->delegator, itr->coining_shares ) );
         ...
         case BSD_SYMBOL:
            if( a.bsd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.bsd_seconds > 0 &&
                   (acnt.bsd_seconds_last_update - acnt.bsd_last_interest_payment).to_seconds() > BEARSHARES_BSD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.bsd_seconds / BEARSHARES_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().bsd_interest_rate;
                  interest /= BEARSHARES_100_PERCENT;
         ...
         case BSD_SYMBOL:
            if( a.savings_bsd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_bsd_seconds > 0 &&
                   (acnt.savings_bsd_seconds_last_update - acnt.savings_bsd_last_interest_payment).to_seconds() > BEARSHARES_BSD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_bsd_seconds / BEARSHARES_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().bsd_interest_rate;
                  interest /= BEARSHARES_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), BSD_SYMBOL);
...
void database::adjust_reward_balance( const account_object& a, const asset& delta )
{
   modify( a, [&]( account_object& acnt )
   {
      ...
      {
         case BEARS_SYMBOL:
            acnt.reward_bears_balance += delta;
            break;
         case BSD_SYMBOL:
            acnt.reward_bsd_balance += delta;
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
      }
...
void database::adjust_supply( const asset& delta, bool adjust_coining )
{
   ...
   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < BEARSHARES_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol )
      {
         ...
         case BSD_SYMBOL:
            ...
            assert( props.current_bsd_supply.amount.value >= 0 );
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
      }
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( BEARSHARES_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( BEARSHARES_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_1 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_1_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_1 ] = BEARSHARES_HARDFORK_0_1_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_2 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_2_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_2 ] = BEARSHARES_HARDFORK_0_2_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_3 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_3_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_3 ] = BEARSHARES_HARDFORK_0_3_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_4 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_4_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_4 ] = BEARSHARES_HARDFORK_0_4_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_5 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_5_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_5 ] = BEARSHARES_HARDFORK_0_5_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_6 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_6_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_6 ] = BEARSHARES_HARDFORK_0_6_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_7 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_7_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_7 ] = BEARSHARES_HARDFORK_0_7_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_8 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_8_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_8 ] = BEARSHARES_HARDFORK_0_8_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_9 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_9_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_9 ] = BEARSHARES_HARDFORK_0_9_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_10 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_10_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_10 ] = BEARSHARES_HARDFORK_0_10_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_11 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_11_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_11 ] = BEARSHARES_HARDFORK_0_11_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_12 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_12_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_12 ] = BEARSHARES_HARDFORK_0_12_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_13 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_13_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_13 ] = BEARSHARES_HARDFORK_0_13_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_14 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_14_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_14 ] = BEARSHARES_HARDFORK_0_14_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_15 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_15_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_15 ] = BEARSHARES_HARDFORK_0_15_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_16 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_16_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_16 ] = BEARSHARES_HARDFORK_0_16_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_17 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_17_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_17 ] = BEARSHARES_HARDFORK_0_17_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_18 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_18_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_18 ] = BEARSHARES_HARDFORK_0_18_VERSION;
   FC_ASSERT( BEARSHARES_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_times[ BEARSHARES_HARDFORK_0_19 ] = fc::time_point_sec( BEARSHARES_HARDFORK_0_19_TIME );
   _hardfork_versions[ BEARSHARES_HARDFORK_0_19 ] = BEARSHARES_HARDFORK_0_19_VERSION;


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= BEARSHARES_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("BEARSHARES_NUM_HARDFORKS",BEARSHARES_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= BEARSHARES_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( BEARSHARES_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ BEARSHARES_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( BEARSHARES_HARDFORK_0_5__54 ) )
      {
         ...
         {
            if( hardforks.last_hardfork < BEARSHARES_NUM_HARDFORKS ) {
      ...
      {
         while( hardforks.last_hardfork < BEARSHARES_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < BEARSHARES_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= BEARSHARES_NUM_HARDFORKS; i++ )
   {
      if( i <= BEARSHARES_HARDFORK_0_5__54 )
   ...
   switch( hardfork )
   {
      case BEARSHARES_HARDFORK_0_1:
...
#ifdef IS_TEST_NET
         {
            ...
            test_op.required_auths.insert( BEARSHARES_INIT_MINER_NAME );
...
#endif
         break;
      case BEARSHARES_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case BEARSHARES_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case BEARSHARES_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case BEARSHARES_HARDFORK_0_5:
         break;
      case BEARSHARES_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case BEARSHARES_HARDFORK_0_7:
         break;
      case BEARSHARES_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case BEARSHARES_HARDFORK_0_9:
         {
         ...
         break;
      case BEARSHARES_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case BEARSHARES_HARDFORK_0_11:
         break;
      case BEARSHARES_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == BEARSHARES_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + BEARSHARES_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + BEARSHARES_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( BEARSHARES_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( BEARSHARES_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( BEARSHARES_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case BEARSHARES_HARDFORK_0_13:
         break;
      case BEARSHARES_HARDFORK_0_14:
         break;
      case BEARSHARES_HARDFORK_0_15:
         break;
      case BEARSHARES_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > BEARSHARES_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case BEARSHARES_HARDFORK_0_17:
         {
            static_assert(
               BEARSHARES_MAX_VOTED_WITNESSES_HF0 + BEARSHARES_MAX_MINER_WITNESSES_HF0 + BEARSHARES_MAX_RUNNER_WITNESSES_HF0 == BEARSHARES_MAX_WITNESSES,
               "HF0 witness counts must add up to BEARSHARES_MAX_WITNESSES" );
            static_assert(
               BEARSHARES_MAX_VOTED_WITNESSES_HF17 + BEARSHARES_MAX_MINER_WITNESSES_HF17 + BEARSHARES_MAX_RUNNER_WITNESSES_HF17 == BEARSHARES_MAX_WITNESSES,
               "HF17 witness counts must add up to BEARSHARES_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = BEARSHARES_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = BEARSHARES_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = BEARSHARES_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = BEARSHARES_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = BEARSHARES_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = BEARSHARES_1_PERCENT * 25;
               rfo.percent_content_rewards = BEARSHARES_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_bears;
#ifndef IS_TEST_NET
               rfo.recent_claims = BEARSHARES_HF_17_RECENT_CLAIMS;
#endif
            ...
            */
            ...
            root_posts.reserve( BEARSHARES_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( BEARSHARES_HF_17_NUM_REPLIES );
            ...
            for( auto itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + BEARSHARES_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( auto itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + BEARSHARES_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case BEARSHARES_HARDFORK_0_18:
         break;
      case BEARSHARES_HARDFORK_0_19:
         {
            ...
            modify( get< reward_fund_object, by_name >( BEARSHARES_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = BEARSHARES_HF_19_RECENT_CLAIMS;
#endif
...
void database::validate_invariants()const
{
   try
   {
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_vsf_votes += ( itr->proxy == BEARSHARES_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( BEARSHARES_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[BEARSHARES_MAX_PROXY_RECURSION_DEPTH - 1] :
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.coining_withdraw_rate  = asset( a.to_withdraw / BEARSHARES_COINING_WITHDRAW_INTERVALS_PRE_HF_16, COINS_SYMBOL );
            if( a.coining_withdraw_rate.amount == 0 )
               a.coining_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < BEARSHARES_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != BEARSHARES_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != BEARSHARES_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != BEARSHARES_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.id, witness_id_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.id )
