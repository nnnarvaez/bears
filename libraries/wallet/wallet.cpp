...
#include <bearshares/wallet/wallet.hpp>
#include <bearshares/wallet/api_documentation.hpp>
#include <bearshares/wallet/reflect_util.hpp>

#include <bearshares/account_by_key/account_by_key_api.hpp>
...
namespace bearshares { namespace wallet {
...
public:
   ...
   variant info() const
   {
      ...
      result["median_bsd_price"] = _remote_db->get_current_median_history_price();
      result["account_creation_fee"] = _remote_db->get_chain_properties().account_creation_fee;
      //result["post_reward_fund"] = fc::variant(_remote_db->get_reward_fund( BEARSHARES_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }
   ...
   variant_object about() const
   {
      ...
      result["blockchain_version"]       = BEARSHARES_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["bears_revision"]           = graphene::utilities::git_revision_sha;
      result["bears_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( graphene::utilities::git_revision_unix_timestamp ) );
   ...
   bool import_key(string wif_key)
   {
      ...
      bearshares::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         bearshares::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         bearshares::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         bearshares::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         bearshares::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < BEARSHARES_MAX_TIME_UNTIL_EXPIRATION );
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      auto minimal_signing_keys = tx.minimize_required_signatures(
         ...
         BEARSHARES_MAX_SIG_CHECK_DEPTH,
         true
         );
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, BEARSHARES_CHAIN_ID );
      }
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         for( const auto& a : accounts ) {
            total_bears += a.balance;
            total_coin  += a.coining_shares;
            total_bsd  += a.bsd_balance;
         ...
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(18) << fc::variant(total_bears).as_string() <<" "
                << std::right << std::setw(26) << fc::variant(total_coin).as_string() <<" "
                << std::right << std::setw(16) << fc::variant(total_bsd).as_string() <<"\n";
         return out.str();
      };
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            if ( i < orders.bids.size() )
            {
               ...
               ss
                  ...
                  << ' ' << setw( spacing ) << asset( orders.bids[i].bsd, BSD_SYMBOL ).to_string()
                  << ' ' << setw( spacing ) << asset( orders.bids[i].bears, BEARS_SYMBOL ).to_string()
            ...
            if ( i < orders.asks.size() )
            {
               ...
               ss << ' ' << setw( spacing ) << orders.asks[i].real_price
                  << ' ' << setw( spacing ) << asset( orders.asks[i].bears, BEARS_SYMBOL ).to_string()
                  << ' ' << setw( spacing ) << asset( orders.asks[i].bsd, BSD_SYMBOL ).to_string()
...
{ try {
   ...
   op.fee = my->_remote_db->get_chain_properties().account_creation_fee * asset( BEARSHARES_CREATE_ACCOUNT_WITH_BEARS_MODIFIER, BEARS_SYMBOL );
...
annotated_signed_transaction wallet_api::delegate_coining_shares( string delegator, string delegatee, asset coining_shares, bool broadcast )
{
   FC_ASSERT( !is_locked() );
 ...
 *  will be controlable by this wallet.
 */
annotated_signed_transaction wallet_api::create_account_delegated( string creator, asset bears_fee, asset delegated_coins, string new_account_name, string json_meta, bool broadcast )
{ try {
   FC_ASSERT( !is_locked() );
   auto owner = suggest_brain_key();
   auto active = suggest_brain_key();
...
annotated_signed_transaction wallet_api::transfer_to_coining(string from, string to, asset amount, bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
annotated_signed_transaction wallet_api::withdraw_coining(string from, asset coining_shares, bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
annotated_signed_transaction wallet_api::set_withdraw_coining_route( string from, string to, uint16_t percent, bool auto_coin, bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
annotated_signed_transaction wallet_api::convert_bsd(string from, asset amount, bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
annotated_signed_transaction wallet_api::claim_reward_balance( string account, asset reward_bears, asset reward_bsd, asset reward_coins, bool broadcast )
{
   FC_ASSERT( !is_locked() );
...
annotated_signed_transaction wallet_api::vote( string voter, string author, string permlink, int16_t weight, bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * BEARSHARES_1_PERCENT;
...
annotated_signed_transaction      wallet_api::send_private_message( string from, string to, string subject, string body, bool broadcast ) {
   ...
   op.id = BEARSHARES_PRIVATE_MESSAGE_COP_ID;
