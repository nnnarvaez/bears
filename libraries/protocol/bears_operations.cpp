...
namespace bearshares { namespace protocol {
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= BEARSHARES_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= BEARSHARES_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= BEARSHARES_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_bears_dollars <= BEARSHARES_100_PERCENT, "Percent cannot exceed 100%" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= BEARSHARES_100_PERCENT, "Weight is not a BEARSHARES percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( memo.size() < BEARSHARES_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   {
      validate_account_name( from );
      FC_ASSERT( is_asset_type( amount, BEARS_SYMBOL ), "Amount must be BEARS" );
      if ( to != account_name_type() ) validate_account_name( to );
      FC_ASSERT( amount > asset( 0, BEARS_SYMBOL ), "Must transfer a nonzero amount" );
   }
   ...
   void set_withdraw_coining_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= BEARSHARES_100_PERCENT, "Percent must be valid bearshares percent" );
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( BEARSHARES_EQUIHASH_N, BEARSHARES_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == BEARSHARES_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == BEARSHARES_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( amount_to_sell, BEARS_SYMBOL ) && is_asset_type( min_to_receive, BSD_SYMBOL ) )
         ...
         "Limit order must be for the BEARS:BSD market" );
