...
struct book
{
     ...
     auth( bip::allocator<bearshares::chain::shared_authority, bip::managed_mapped_file::segment_manager>( al.get_segment_manager() )),
     ...
     int32_t                          pages;
     int32_t                          prize;
     bearshares::chain::shared_authority auth;
...
int main(int argc, char** argv, char** envp)
{
   ...
   */
   ...
   //b.pages = pbc->size();
   //b.auth = bearshares::chain::authority( 1, "dan", pbc->size() );
   pbc->emplace( [&]( book& b ) {
                 b.name = "emplace name";
                 b.pages = pbc->size();
