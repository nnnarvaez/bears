...
using bearshares::protocol::calc_percent_reward_per_block;
using bearshares::protocol::calc_percent_reward_per_round;
using bearshares::protocol::calc_percent_reward_per_hour;
...
#define BEARSHARES_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/
   ...
   {
      uint32_t coining_factor = (block_num < BEARSHARES_START_COINING_BLOCK) ? 0 : 9;

      share_type curate_reward   = calc_percent_reward_per_block< BEARSHARES_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, BEARSHARES_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * coining_factor;

      share_type content_reward  = calc_percent_reward_per_block< BEARSHARES_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, BEARSHARES_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * coining_factor;

      share_type producer_reward = calc_percent_reward_per_block< BEARSHARES_PRODUCER_APR_PERCENT >( current_supply );
      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, BEARSHARES_MIN_PRODUCER_REWARD.amount );
      ...
      if( (block_num % BEARSHARES_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< BEARSHARES_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, BEARSHARES_MIN_POW_REWARD.amount );
         if( block_num < BEARSHARES_START_MINER_VOTING_BLOCK )
            pow_reward *= BEARSHARES_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % BEARSHARES_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< BEARSHARES_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, BEARSHARES_MIN_LIQUIDITY_REWARD.amount );
      }
   ...
   for( uint32_t b=1; b<10*BEARSHARES_BLOCKS_PER_YEAR; b++ )
   {
