...
#include <bearshares/manifest/plugins.hpp>
...
int main(int argc, char** argv) {
   bearshares::plugin::initialize_plugin_factories();
   ...
   try {
      ...
      std::cerr << "------------------------------------------------------\n";
      auto bearshare_private_key = graphene::utilities::key_to_wif( BEARSHARES_INIT_PRIVATE_KEY );
      std::cerr << "bearshare public key: " << BEARSHARES_INIT_PUBLIC_KEY_STR << "\n";
      std::cerr << "bearshare private key: " << bearshare_private_key << "\n";
      std::cerr << "chain id: " << std::string(BEARSHARES_CHAIN_ID) << "\n";
      std::cerr << "blockchain version: " << fc::string( BEARSHARES_BLOCKCHAIN_VERSION ) << "\n";
      std::cerr << "------------------------------------------------------\n";
#else
      std::cerr << "------------------------------------------------------\n\n";
      ...
      bpo::options_description app_options("Bears Daemon");
      bpo::options_description cfg_options("Bears Daemon");
      app_options.add_options()
            ("help,h", "Print this help message and exit.")
            ("data-dir,d", bpo::value<boost::filesystem::path>()->default_value("witness_node_data_dir"), "Directory containing databases, configuration file, etc.")
            ("version,v", "Print bearsd version and exit.")
            ;

      bpo::variables_map options;

      for( const std::string& plugin_name : bearshares::plugin::get_available_plugins() )
         node->register_abstract_plugin( bearshares::plugin::create_plugin( plugin_name, node ) );
      ...
      {
         std::cout << "bears_blockchain_version: " << fc::string( BEARSHARES_BLOCKCHAIN_VERSION ) << "\n";
         std::cout << "bears_git_revision:       " << fc::string( graphene::utilities::git_revision_sha ) << "\n";
