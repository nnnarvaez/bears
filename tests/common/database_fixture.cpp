...
//using namespace bearshares::chain::test;

uint32_t BEARSHARES_TESTING_GENESIS_TIMESTAMP = 1431700000;
...
clean_database_fixture::clean_database_fixture()
{
   ...
   auto wit_plugin = app.register_plugin< bearshares::witness::witness_plugin >();
   ...
   generate_block();
   db.set_hardfork( BEARSHARES_NUM_HARDFORKS );
   generate_block();
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db.head_block_time() - timestamp ).to_seconds() < BEARSHARES_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      if( db.has_hardfork( BEARSHARES_HARDFORK_0_17 ) )
      {
...
)
{
   try
   {
      return account_create(
         name,
         BEARSHARES_INIT_MINER_NAME,
         init_account_priv_key,
         std::max( db.get_witness_schedule_object().median_props.account_creation_fee.amount * BEARSHARES_CREATE_ACCOUNT_WITH_BEARS_MODIFIER, share_type( 100 ) ),
         key,
         post_key,
         "" );
   }
   ...
   )
{
   try
   {
      transfer( BEARSHARES_INIT_MINER_NAME, account_name, amount );
   ...
   const asset& amount )
{
   try
   {
      ...
      if ( amount.symbol == BEARS_SYMBOL )
      {
         ...
         db.adjust_balance( account, db.to_bsd( amount ) );
      ...
      {
         db.adjust_balance( account, -amount );
         db.adjust_balance( account, db.to_bears( amount ) );
         db.adjust_supply( -amount );
...
void database_fixture::coin( const string& account, const asset& amount )
{
   if( amount.symbol != BEARS_SYMBOL )
      return;
   ...
   db_plugin->debug_update( [=]( database& db )
   {
      ...
      db.create_coining( db.get_account( account ), amount );
...
void database_fixture::set_price_feed( const price& new_price )
{
   try
   {
      for ( int i = 1; i < 8; i++ )
      {
         ...
         op.publisher = BEARSHARES_INIT_MINER_NAME + fc::to_string( i );
   ...
   generate_blocks( BEARSHARES_BLOCKS_PER_HOUR );
   BOOST_REQUIRE(
#ifdef IS_TEST_NET
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   {
      itr--;
      ops.push_back( fc::raw::unpack< bearshares::chain::operation >( db.get(itr->op).serialized_op ) );
   }
