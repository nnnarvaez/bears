...
extern uint32_t ( BEARSHARES_TESTING_GENESIS_TIMESTAMP );

#define PUSH_TX \
   bearshares::chain::test::_push_transaction

#define PUSH_BLOCK \
   bearshares::chain::test::_push_block
...
/*#define BEARSHARES_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BEARSHARES_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BEARSHARES_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define BEARSHARES_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BEARSHARES_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "BEARSHARES_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   BEARSHARES_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   BEARSHARES_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
struct database_fixture {
   ...
   std::shared_ptr< bearshares::plugin::debug_node::debug_node_plugin > db_plugin;
   ...
   void transfer( const string& from, const string& to, const share_type& bears );
   void convert( const string& account_name, const asset& amount );
   void coin( const string& from, const share_type& amount );
   void coin( const string& account, const asset& amount );
