...
#include <bearshares/plugins/debug_node/debug_node_plugin.hpp>
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = BEARSHARES_ROOT_POST_PARENT;
      ...
      generate_blocks(10);
      //const auto& rf = db.get< reward_fund_object, by_name >( BEARSHARES_POST_REWARD_FUND_NAME );
      //idump( (rf) );
      ...
      */
      ...
      BOOST_CHECK( alice_account.reward_bsd_balance == ASSET( "14288.000 TBD" ) );
...
BOOST_AUTO_TEST_CASE( comment_payout_dust )
{
   try
   {
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.weight = 81 * BEARSHARES_1_PERCENT;
      ...
      vote.voter = "bob";
      vote.author = "bob";
      vote.weight = 59 * BEARSHARES_1_PERCENT;
...
BOOST_AUTO_TEST_CASE( reward_funds )
{
   try
   {
      ...
      {
         const auto& post_rf = db.get< reward_fund_object, by_name >( BEARSHARES_POST_REWARD_FUND_NAME );
         const auto& comment_rf = db.get< reward_fund_object, by_name >( BEARSHARES_COMMENT_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto alice_vshares = util::evaluate_reward_curve( db.get_comment( "alice", string( "test" ) ).net_rshares.value,
         db.get< reward_fund_object, by_name >( BEARSHARES_POST_REWARD_FUND_NAME ).author_reward_curve,
         db.get< reward_fund_object, by_name >( BEARSHARES_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * BEARSHARES_BLOCK_INTERVAL ) / BEARSHARES_RECENT_RSHARES_DECAY_RATE_HF19.to_seconds();
         const auto& post_rf = db.get< reward_fund_object, by_name >( BEARSHARES_POST_REWARD_FUND_NAME );
...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      //generate_blocks( db.get_comment( "bob", string( "test" ) ).cashout_time - BEARSHARES_BLOCK_INTERVAL, true );
      ...
      vote.voter = "dave";
      vote.author = "bob";
      vote.weight = BEARSHARES_1_PERCENT;
      tx.operations.push_back( vote );
      tx.sign( dave_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db.head_block_time().sec_since_epoch() + BEARSHARES_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db.get_comment( "bob", string( "test" ) ).cashout_time - ( BEARSHARES_BLOCK_INTERVAL / 2 ), true );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - ( BEARSHARES_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = BEARSHARES_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      tx.sign( bob_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( BEARSHARES_BLOCK_INTERVAL ), true );
...
BOOST_AUTO_TEST_CASE( coining_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db.head_block_time() + BEARSHARES_COINING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( BEARSHARES_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < BEARSHARES_COINING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db.head_block_time() + BEARSHARES_COINING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == BEARSHARES_COINING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_coining_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_coining_withdrawal.sec_since_epoch() == ( old_next_coining + BEARSHARES_COINING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      if (  to_withdraw.amount.value % withdraw_rate.amount.value != 0 )
      {
         ...
         generate_blocks( db.head_block_time() + BEARSHARES_COINING_WITHDRAW_INTERVAL_SECONDS, true );
         fill_op = get_last_operations( 1 )[0].get< fill_coining_withdraw_operation >();
         gpo = db.get_dynamic_global_properties();

         BOOST_REQUIRE( db.get_account( "alice" ).next_coining_withdrawal.sec_since_epoch() == ( old_next_coining + BEARSHARES_COINING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
         BOOST_REQUIRE( fill_op.from_account == "alice" );
         BOOST_REQUIRE( fill_op.to_account == "alice" );
         BOOST_REQUIRE( fill_op.withdrawn.amount.value == withdraw_rate.amount.value );
         BOOST_REQUIRE( std::abs( ( fill_op.deposited - fill_op.withdrawn * gpo.get_coining_share_price() ).amount.value ) <= 1 );
...
BOOST_AUTO_TEST_CASE( coining_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = BEARSHARES_1_PERCENT * 50;
      op.auto_coin = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = BEARSHARES_1_PERCENT * 30;
      op.auto_coin = false;
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( coining_withdraw_rate.amount * BEARSHARES_1_PERCENT * 20 ) / BEARSHARES_100_PERCENT, COINS_SYMBOL ) * db.get_dynamic_global_properties().get_coining_share_price() );
         BOOST_REQUIRE( bob.coining_shares == old_bob_coining + asset( ( coining_withdraw_rate.amount * BEARSHARES_1_PERCENT * 50 ) / BEARSHARES_100_PERCENT, COINS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.coining_shares == old_sam_coining );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( coining_withdraw_rate.amount * BEARSHARES_1_PERCENT * 30 ) / BEARSHARES_100_PERCENT, COINS_SYMBOL ) * db.get_dynamic_global_properties().get_coining_share_price() );
      ...
      op.to_account = "sam";
      op.percent = BEARSHARES_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db.head_block_time() + BEARSHARES_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db.get_chain_id() );
      ...
      generate_blocks( db.get_account( "alice" ).next_coining_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.coining_shares == old_sam_coining );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( coining_withdraw_rate.amount * BEARSHARES_1_PERCENT * 50 ) / BEARSHARES_100_PERCENT, COINS_SYMBOL ) * db.get_dynamic_global_properties().get_coining_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / BEARSHARES_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( BEARSHARES_INIT_MINER_NAME, accounts[i], 10000 );
      ...
      ops[0].exchange_rate = price( asset( 100000, BEARS_SYMBOL ), asset( 1000, BSD_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 105000, BEARS_SYMBOL ), asset( 1000, BSD_SYMBOL ) );
      ops[2].exchange_rate = price( asset(  98000, BEARS_SYMBOL ), asset( 1000, BSD_SYMBOL ) );
      ops[3].exchange_rate = price( asset(  97000, BEARS_SYMBOL ), asset( 1000, BSD_SYMBOL ) );
      ops[4].exchange_rate = price( asset(  99000, BEARS_SYMBOL ), asset( 1000, BSD_SYMBOL ) );
      ops[5].exchange_rate = price( asset(  97500, BEARS_SYMBOL ), asset( 1000, BSD_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 102000, BEARS_SYMBOL ), asset( 1000, BSD_SYMBOL ) );

      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db.head_block_time() + BEARSHARES_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( BEARSHARES_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 99000, BEARS_SYMBOL), asset( 1000, BSD_SYMBOL ) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 99000, BEARS_SYMBOL), asset( 1000, BSD_SYMBOL ) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            ops[j].exchange_rate = price( ops[j].exchange_rate.base, asset( ops[j].exchange_rate.quote.amount + 10, BSD_SYMBOL ) );
            txs[j].set_expiration( db.head_block_time() + BEARSHARES_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + BEARSHARES_CONVERSION_DELAY - fc::seconds( BEARSHARES_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( bears_inflation )
{
   ...
   /*
      ...
      auto new_rewards = std::max( BEARSHARES_MIN_CONTENT_REWARD, asset( ( BEARSHARES_CONTENT_APR * gpo.virtual_supply.amount ) / ( BEARSHARES_BLOCKS_PER_YEAR * 100 ), BEARS_SYMBOL ) )
         + std::max( BEARSHARES_MIN_CURATE_REWARD, asset( ( BEARSHARES_CURATE_APR * gpo.virtual_supply.amount ) / ( BEARSHARES_BLOCKS_PER_YEAR * 100 ), BEARS_SYMBOL ) );
      auto witness_pay = std::max( BEARSHARES_MIN_PRODUCER_REWARD, asset( ( BEARSHARES_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BEARSHARES_BLOCKS_PER_YEAR * 100 ), BEARS_SYMBOL ) );
      ...
      while( db.head_block_num() < BEARSHARES_START_COINING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( BEARSHARES_MIN_CONTENT_REWARD, asset( ( BEARSHARES_CONTENT_APR * gpo.virtual_supply.amount ) / ( BEARSHARES_BLOCKS_PER_YEAR * 100 ), BEARS_SYMBOL ) )
            + std::max( BEARSHARES_MIN_CURATE_REWARD, asset( ( BEARSHARES_CURATE_APR * gpo.virtual_supply.amount ) / ( BEARSHARES_BLOCKS_PER_YEAR * 100 ), BEARS_SYMBOL ) );
         witness_pay = std::max( BEARSHARES_MIN_PRODUCER_REWARD, asset( ( BEARSHARES_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BEARSHARES_BLOCKS_PER_YEAR * 100 ), BEARS_SYMBOL ) );
      ...
      while( db.head_block_num() < BEARSHARES_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
      ...
      for( int i = 0; i < BEARSHARES_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_coining_fund_bears.amount.value,
         ( coining_bears.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / BEARSHARES_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / BEARSHARES_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_bears.amount.value,
         reward_bears.amount.value + virtual_supply.amount.value / 10 / BEARSHARES_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / BEARSHARES_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).coining_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / BEARSHARES_BLOCKS_PER_YEAR ) * BEARSHARES_1_PERCENT ) / BEARSHARES_100_PERCENT, BEARS_SYMBOL ) * ( coining_shares / coining_bears ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( bsd_interest )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + fc::seconds( BEARSHARES_BSD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).bsd_balance.amount.value == alice_bsd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_bsd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() ) / BEARSHARES_SECONDS_PER_YEAR ) * gpo.bsd_interest_rate ) / BEARSHARES_100_PERCENT ).to_uint64() );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( BEARSHARES_BSD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( ( BEARSHARES_BSD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).bsd_balance.amount.value == alice_bsd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_bsd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() + alice_coindays ) / BEARSHARES_SECONDS_PER_YEAR ) * gpo.bsd_interest_rate ) / BEARSHARES_100_PERCENT ).to_uint64() );
      validate_database();
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( liquidity_rewards )
{
   ...
   try
   {
      ...
      generate_blocks( db.head_block_time() + BEARSHARES_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10, true );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( BEARSHARES_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10.to_seconds() / 2 ), true );
      ...
      generate_blocks( BEARSHARES_LIQUIDITY_REWARD_BLOCKS - ( db.head_block_num() % BEARSHARES_LIQUIDITY_REWARD_BLOCKS ) - 1 );

      BOOST_REQUIRE( db.head_block_num() % BEARSHARES_LIQUIDITY_REWARD_BLOCKS == BEARSHARES_LIQUIDITY_REWARD_BLOCKS - 1 );
      ...
      //alice_balance += BEARSHARES_MIN_LIQUIDITY_REWARD;
      ...
      BEARSHARES_REQUIRE_THROW( ops[0].get< liquidity_reward_operation>(), fc::exception );
      //BOOST_REQUIRE( ops[0].get< liquidity_reward_operation>().payout.amount.value == BEARSHARES_MIN_LIQUIDITY_REWARD.amount.value );

      generate_blocks( BEARSHARES_LIQUIDITY_REWARD_BLOCKS );

      //bob_balance += BEARSHARES_MIN_LIQUIDITY_REWARD;
      ...
      generate_blocks( sam_reward_last_update + BEARSHARES_LIQUIDITY_TIMEOUT_SEC - fc::seconds( BEARSHARES_BLOCK_INTERVAL / 2 ) - BEARSHARES_MIN_LIQUIDITY_REWARD_PERIOD_SEC , true );
      ...
      generate_blocks( db.head_block_time() + ( BEARSHARES_BLOCK_INTERVAL / 2 ) + BEARSHARES_LIQUIDITY_TIMEOUT_SEC, true );
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test1" ) ).reward_weight == BEARSHARES_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db.head_block_time() + BEARSHARES_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( BEARSHARES_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test2" ) ).reward_weight == BEARSHARES_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test3" ) ).reward_weight == BEARSHARES_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test4" ) ).reward_weight == BEARSHARES_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test5" ) ).reward_weight == BEARSHARES_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = BEARSHARES_100_PERCENT * -1;
...
BOOST_AUTO_TEST_CASE( bsd_stability )
{
   try
   {
      ...
      auto db_plugin = app.register_plugin< bearshares::plugin::debug_node::debug_node_plugin >();
      ...
      BOOST_REQUIRE( db.get_dynamic_global_properties().bsd_print_rate == BEARSHARES_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db.get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * BEARSHARES_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db.get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset bsd_balance = asset( ( gpo.virtual_supply.amount * ( BEARSHARES_BSD_STOP_PERCENT + 30 ) ) / BEARSHARES_100_PERCENT, BEARS_SYMBOL ) * exchange_rate;
      ...
      auto comment_reward = ( gpo.total_reward_fund_bears.amount + 2000 ) - ( ( gpo.total_reward_fund_bears.amount + 2000 ) * 25 * BEARSHARES_1_PERCENT ) / BEARSHARES_100_PERCENT ;
      comment_reward /= 2;
      auto bsd_reward = ( comment_reward * gpo.bsd_print_rate ) / BEARSHARES_100_PERCENT;
      ...
      BOOST_TEST_MESSAGE( "Letting percent market cap fall to 2% to verify printing of BSD turns back on" );

      // Get close to 1.5% for printing BSD to start again, but not all the way
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.bsd_balance = asset( ( 194 * bsd_balance.amount ) / 500, BSD_SYMBOL );
         });
      }, database::skip_witness_signature );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.current_bsd_supply = alice_bsd + asset( ( 194 * bsd_balance.amount ) / 500, BSD_SYMBOL );
         });
      }, database::skip_witness_signature );
      ...
      while( ( db.get_dynamic_global_properties().current_bsd_supply * exchange_rate ).amount >= ( db.get_dynamic_global_properties().virtual_supply.amount * BEARSHARES_BSD_START_PERCENT ) / BEARSHARES_100_PERCENT )
      {
         auto& gpo = db.get_dynamic_global_properties();
...
BOOST_AUTO_TEST_CASE( bsd_price_feed_limit )
{
   try
   {
      ...
      auto new_exchange_rate = price( gpo.current_bsd_supply, asset( ( BEARSHARES_100_PERCENT ) * gpo.current_supply.amount ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( BEARSHARES_NULL_ACCOUNT ), [&]( account_object& a )
         {
      ...
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).bsd_balance == ASSET( "2.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).coining_shares > ASSET( "0.000000 COINS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).savings_bsd_balance == ASSET( "5.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_bsd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_bears_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_coining_balance == ASSET( "1.000000 COINS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_coining_bears == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).bsd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).coining_shares == ASSET( "0.000000 COINS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).savings_bsd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_bsd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_bears_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_coining_balance == ASSET( "0.000000 COINS" ) );
      BOOST_REQUIRE( db.get_account( BEARSHARES_NULL_ACCOUNT ).reward_coining_bears == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "alice" ).bsd_balance == ASSET( "3.000 TBD" ) );
   }
   FC_LOG_AND_RETHROW()
